...
async function handleCheckoutSessionCompleted(
  ...
  if (!userId) {
    console.error(
      "❌ [ERROR] clerkUserId is missing from session.metadata. Cannot process subscription."
    );
    console.error(
      "❌ [ERROR] Available metadata keys:",
      session.metadata
  ...
  if (!subscriptionId) {
    console.error(
      "❌ [ERROR] No subscription ID in session. Session ID:",
      session.id
    );
    // Return without throwing to prevent Stripe from retrying endlessly
    return;
  ...
  if (!email) {
    console.error(
      "❌ [ERROR] No customer email in session. Cannot create profile."
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      "✅ Profile upserted successfully - Subscription activated for user:",
      userId
    );
...
async function handleInvoicePaymentFailed(invoice: Stripe.Invoice) {
  ...
  try {
    ...
    if (profile) {
      ...
      console.log(
        "✅ Subscription deactivated due to payment failure - User:",
        profile.userId
      );
    } else {
      console.log("⚠️ Profile not found for subscription:", subscriptionId);
...
async function handleSubscriptionDeleted(subscription: Stripe.Subscription) {
  ...
  try {
    ...
    if (profile) {
      ...
      console.log(
        "✅ Subscription deleted - Profile deactivated for user:",
        profile.userId
      );
    } else {
      console.log("⚠️ Profile not found for subscription:", subscriptionId);
